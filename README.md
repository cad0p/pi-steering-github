...
The gate keys on `-R/--repo` **PRESENCE, not position**: covered are `-R x/y`, `--repo x/y`, `--repo=x/y`, `-Rx/y` in ANY of the leading flag(+value) pairs (#41 lifted the one-pair cap — `gh --hostname h -R x/y pr merge` routes) OR anywhere on the gated subcommand line. A command carrying NO `-R/--repo` anywhere is released untouched — the vault-body/keyword policies apply as usual. Non-repo leading flags (`-v`, `--hostname`) route but release on absence; note that a real repo flag after them now blocks (`-v … --repo=<foreign>` escaped the gate before #39). The anchor's value arm ignores a bare-dash value token (`gh -F - pr create` stays unrouted) — the accepted cost of its linear-time, ReDoS-proof guard. The gate is **fully declarative** — zero condition code, an AND of two registered-predicate leaves:
