...
Since #43 failed byte-pins and token deviations carry a byte-exact diff: a mutated strip program reports the divergent core spans with the byte offset (`substitution program diverges from the pinned strip at byte 129: - expected: *)? / + got: ?)*`) and token-count deviations report the positional token mismatch — always followed by the canonical recipe, so the block message stays actionable.
