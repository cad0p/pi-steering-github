...
`gh -R x/y pr create|new|edit|merge`, `gh pr merge --repo=cad0p/x …`, and every other gated `pr|issue …` mutation that **carries** `-R/--repo` targeting a foreign repo is blocked with a redirect: run a foreign subagent maintainer loop until good, then cd into the foreign repo and target it from there. This is the ENTRY step of the foreign flow — the rule sits FIRST in the roster so the redirect is the first thing the agent meets. Its router anchor now OVERLAPS the other rules' `^gh\s+(?:pr|issue)` anchors (it routes gated subcommands with zero or one leading flag — both `-R` positions since #39), so correctness rests on the evaluator's first-firing-rule-wins ordering plus RELEASE FALL-THROUGH: the `foreignRepoTarget` predicate releases every command without a foreign target and the per-subcommand rules evaluate normally.

The gate keys on `-R/--repo` **PRESENCE, not position**: covered are `-R x/y`, `--repo x/y`, `--repo=x/y`, `-Rx/y` as the SOLE leading flag OR anywhere on the gated subcommand line. A command carrying NO `-R/--repo` anywhere is released untouched — the vault-body/keyword policies apply as usual. Non-repo leading flags (`-v`, `--hostname`) route but release on absence; note that a real repo flag after them now blocks (`-v … --repo=<foreign>` escaped the gate before #39). Two leading flags (`gh --hostname h -R x/y pr merge`) stay unrouted — boundary documented under Known limitations. The gate is **fully declarative** — zero condition code, an AND of two registered-predicate leaves:
...
- **Two leading flags are not routed**: the router covers gated subcommands with ZERO or ONE leading flag(+value) — `gh --hostname h -R x/y pr merge` escapes the foreign gate entirely (pre-existing boundary; #39 narrowed the unrouted class from "any `-R` after the subcommand" to just this shape).
